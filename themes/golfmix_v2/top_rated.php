...
foreach($nearby as $post_id) {
		...
		if($style == 'page') {
				$query = "SELECT comment_id 
				  ...
				  WHERE wp_comments.comment_post_ID = $post_id 
				  	...
				  	AND wp_comments.comment_date LIKE '%$year%'
				  ";
		...
		if($year == '2011') {
			if($number < 10 && $api_search !== 'true' && $market_coverage !== 'CA') { continue;}
		} else {
			if($number < 7 && $api_search !== 'true' && $market_coverage !== 'CA') { continue;}
		}
		
		if($post_id == $exclude){ continue; }

		if($style == 'page') {
			$ratings = get_ratings_by_year($post_id, $year);
		} else { 

...
if(isset($_REQUEST['full'])) {  
	setcookie("FullSite", "View Full Site", time()+3600); 
	header("Location: http://golfmix.com/arizona");
	exit;
}
...
<meta name="description" content="<?php if(is_single() && get_post_type() == 'post') { echo 'Reviews of '.get_the_title().', from regular golfers just like you.  Read & write your own reviews at www.golfmix.com.'; } elseif(is_single()) { setup_postdata($post); echo strip_tags(get_the_excerpt($post->ID)); } else { echo "golfmix is the ultimate resource to help you decide where to play golf in ".$market_name.".  read and write reviews of every course in the state.  Whether you're a scratch or you shoot 115, there are golfers like you sharing their thoughts and ratings of tracks all across ".$market_name.".  Join the movement!"; } ?>">	
...
if ($detect->isMobile() && !$detect->isIpad()) { ?>
	...
	<link rel="stylesheet" href="<?php bloginfo('template_url'); ?>/style-mobile.css?v=2.0" type="text/css" media="screen" />
	<script type="text/javascript" src="http://ajax.googleapis.com/ajax/libs/jquery/1.4.2/jquery.min.js"></script>
	
	<?php wp_head(); ?>
	
	<?php if(is_page('golf-courses') || !stristr($this_uri, 'golf-courses') === FALSE) { ?>
...
<?php if(is_page('golf-courses') || !stristr($this_uri, 'golf-courses') === FALSE) { ?><body onload="initialize()" onunload="GUnload()"><?php } else { ?><body><?php } ?>
	...
	<div id="header"><a href="<?php bloginfo('url'); ?>" id="logo">golfmix</a><?php if(stristr($this_uri, 'arizona') === FALSE) { ?><a href="<?php echo $_SERVER['HTTP_REFERER']; ?>?app=no" class="back">&#171; Back</a><?php } ?><?php if(is_page('Search') && stristr($this_uri,'map') === FALSE) { ?><a href="<?php echo get_bloginfo('url').'/golf-courses/?'.$_SERVER['QUERY_STRING']; ?>&map=1" class="back" style="right: 10px;left: auto;">Map</a><?php } elseif(is_page('Search')) { ?><a href="<?php echo get_bloginfo('url').'/golf-courses/?'.str_replace('&map=1','',$_SERVER['QUERY_STRING']); ?>" class="back" style="right: 10px;left: auto;">List</a><?php } ?>
</div>
	
	<?php if(is_page('819')) { } else { ?>
	<div id="search">
		<form method="get" class="mobile-search" action="<?php bloginfo('url'); ?>/golf-courses/" autocomplete="off">
...
<link rel="stylesheet" type="text/css" media="all" href="<?php bloginfo( 'stylesheet_url' ); ?>?v=2.5" />
...
<meta property="og:description"
      content="<?php if(is_page()) { the_title(); } elseif(is_single() && get_post_type() == 'post') { echo 'Reviews of '.get_the_title().', from regular golfers just like you.  Read & write your own reviews at www.golfmix.com.'; } elseif(is_single()) { setup_postdata($post); echo strip_tags(get_the_excerpt($post->ID)); } else { echo "golfmix is the ultimate resource to help you decide where to play golf in ".$market_name.".  read and write reviews of every course in the state.  whether you're a scratch or you shoot 115, there are golfers like you sharing their thoughts and ratings of tracks all across ".$market_name.".  join the movement!"; } ?>"/>
...
<link rel="stylesheet" type="text/css" media="all" href="<?php bloginfo( 'template_url' ); ?>/stylesheet.css?v=2.0" />
...
GA_googleAddSlot("ca-pub-2177055306714894", "Homepage_300x250");
</script>
<script type='text/javascript'>
GA_googleFetchAds();
</script>
...
<?php 
if((is_single() && get_post_type() == 'post') || is_page('golf-courses') || !stristr($this_uri, 'golf-courses') === FALSE) { ?><body onload="initialize()" onunload="GUnload()"><?php } else { ?><body><?php } ?>

<?php } ?>

<div id="wrapper">

 	<div id="location"><a href="<?php bloginfo('url'); ?>"><?php echo $market_name; ?></a></div>
	...
	<div id="inner-wrapper">
		...
		<div id="header">
			...
			<div class="logo"><a href="<?php bloginfo( 'home' ); ?>/"><img src="<?php bloginfo( 'template_url' ); ?>/images/web-logo.png" alt="" border="0" /></a></div>
			...
			<form method="get" class="custom_search widget custom_search_custom_fields__search" action="<?php bloginfo('url');?>/golf-courses/" autocomplete="off">
		...
		<?php if($turnthisoff=='yes') { ?>
			<div class="featured_giveaway">
				<?PHP if(function_exists('register_sidebar')&&dynamic_sidebar('Featured Header')){} else { ?>
				<strong><strong><a href="http://golfmix.com/blog/welcome-to-golfmix/">Welcome to <?php echo $market_name; ?> golfmix!</a></strong> Tomorrow's <strong>review of the day </strong> will win an Arron Oberholser autographed golfmix hat. To enter, <a href="http://golfmix.com/write-a-review">write a review</a>!
				<?php } ?>
			</div>
		<?php } ?>

...
// Make Categories/Tags work for custom post types
...
//	    $post_type = $post_type;
//	else
//	    $post_type = array('blog','news'); // replace cpt to your custom post type
...
/*Custom Rewrite Rules*/
	...
	function add_rewrite_rules($aRules) {
		$aNewRules = array(
		'golf-courses/sort/([^/]+)/?$' => 'index.php?pagename=golf-courses&sortby=$matches[1]',
		'golf-courses/([^/]+)/?$' => 'index.php?pagename=golf-courses&q=$matches[1]',
		'golf-courses/(.*?)/page/([^/]+)/?$' => 'index.php?pagename=golf-courses&q=$matches[1]&paged=$matches[2]'
		//'golf-courses/([^/]+)/?$' => 'index.php?pagename=golf-courses&location=$matches[1]',		
		);
...
// function to change canonical for search
	function wpseo_canonical_exclude( $canonical ) {
			global $post;
			if ( $post->post_name == 'golf-courses') {
	   			if(get_query_var( 'q' )){
	    			$canonical = get_bloginfo('url').'/'.$post->post_name.'/'.str_replace(' ','-',get_query_var( 'q' ));
	    		} elseif(get_query_var( 'sortby' )) {
	    			$canonical = get_bloginfo('url').'/'.$post->post_name.'/sort/'.str_replace(' ','-',get_query_var( 'sortby' ));
	    		} else {
	    			$canonical = get_bloginfo('url').'/'.$post->post_name;
	    		}
	    		if(get_query_var( 'paged' )) { $canonical .= '/page/'.get_query_var( 'paged' ); }
	   		} 		
	   		return $canonical;
	}
	...
	function wpseo_title_change( $title ) {
			global $post;
			if(get_bloginfo('name') == 'golfmix.com') { $blogname = 'Arizona'; } else { $blogname = get_bloginfo('name');}
			if ( $post->post_name == 'golf-courses') {
				if(get_query_var( 'q' )){
	    			$title = $blogname.' golf courses | '. ucwords(get_query_var( 'q' )).' golf courses';	    		
	    		} elseif(get_query_var( 'sortby' )) {
	    			$title = 'Arizona "'.ucwords(get_query_var( 'sortby' )).'" golf courses';
	    		} else {
	    			$title = $blogname.' golf courses and reviews';
	    		}
			} elseif (is_category('arizona')) {
				$title = 'Arizona golf course reviews | leading source AZ golf reviews';
			}
		return $title;
	}
	add_filter( 'wpseo_title', 'wpseo_title_change' );
	
	
// Turn off registration email? 
add_filter( 'wpmu_signup_user_notification', '__return_false' );

// Print/Display Top Courses
function print_top_courses($url=null) {
	...
	if($data) { return $data; } else { return 'No Top Courses'; }
}

function download_flickr_images_locally($pid,$ptitle=null,$file,$post_id){
	...
	if ( !empty($file) ) { 
		...
		$downloaded_flickr_images = get_post_meta( $post_id, 'downloaded_flickr_images');
		if($downloaded) { 
			$downloaded = explode(',',$downloaded_flickr_images); 
			if(in_array($pid, $downloaded)) { return; }
		} 
		...
		if ( ! update_post_meta ($post_id, 'downloaded_flickr_images', $downloaded_flickr_images.','.$pid ) ) { add_post_meta( $post_id, 'downloaded_flickr_images', $pid ); }

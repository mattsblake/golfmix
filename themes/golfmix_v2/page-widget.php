...
<style>
	...
	.grey_small h1 {
	    ...
	    height: 35px;
		background: #FDFDFD;
		background: -moz-linear-gradient(top, #FDFDFD 0%, #F4F4F4 49%, #EEE 100%);
		background: -webkit-gradient(linear, left top, left bottom, color-stop(0%,#FDFDFD), color-stop(49%,#F4F4F4), color-stop(100%,#EEE));
		background: -webkit-linear-gradient(top, #FDFDFD 0%,#F4F4F4 49%,#EEE 100%);
		background: -o-linear-gradient(top, #FDFDFD 0%,#F4F4F4 49%,#EEE 100%);
		background: -ms-linear-gradient(top, #FDFDFD 0%,#F4F4F4 49%,#EEE 100%);
		background: linear-gradient(top, #FDFDFD 0%,#F4F4F4 49%,#EEE 100%);
...
if($w_id !== 'Missing ID') {
	...
	foreach( $myposts as $post ) :	setup_postdata($post);
		...
		//print_r($ratings);
		if(isset($_REQUEST['sortby'])) { $ratings = $ratings[$_REQUEST['sortby']]; }
		else { $ratings = $ratings["<strong>Overall Experience</strong>"]; }
	?>
	<div id="widget" class="<?php echo $w_style; ?>">
		<a href="<?php the_permalink(); ?>" target="_blank">
		<h1><div id="logo_small"></div><?php if(isset($_REQUEST['rank']) && $_REQUEST['style']=='g') { echo '<div class="best-of-number">#'.$_REQUEST['rank'].'</div>'; } ?><?php if(isset($_REQUEST['best'])) { } else { echo 'Recommended On';} ?></h1>
		...
		<?php } elseif($w_type) { ?>
			...
			<div class="naked_rating"><?php echo num_to_stars_mobile($ratings); ?></div>
		<?php } else { ?>
			<?php if(isset($_REQUEST['best']) && $_REQUEST['style']=='g') { echo '<div class="review_snippet">'.str_replace("2012","2011",stripslashes($_REQUEST['best'])).'</div>'; } ?>
			<div class="naked_rating"><?php echo num_to_stars_mobile($ratings); ?></div>
		<?php } ?>

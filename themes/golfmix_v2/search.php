...
if ($detect->isMobile() && !$detect->isIpad()) { 
	...
	?> 	
		 	<div class="post_nav">
				<?php if($paged - 1 !== 0) { $previouspage = $paged-1; ?><a href="<?php echo get_bloginfo('url').'/golf-courses/'.$q.'/page/'.$previouspage .'?page='.$previouspage; ?><?php if(isset($_REQUEST['q'])) { echo '&q='.$_REQUEST['q']; } ?><?php if(isset($_REQUEST['lat'])) { echo '&lat='.$_REQUEST['lat']; } ?><?php if(isset($_REQUEST['lon'])) { echo '&lon='.$_REQUEST['lon']; } ?><?php if(isset($_REQUEST['map'])) { echo '&map=1'; } ?>" class="next-page">previous page</a><?php } ?>
				<?php if($paged < $search_query->max_num_pages) { $nextpage = $paged+1; ?><a href="<?php echo get_bloginfo('url').'/golf-courses/'.$q.'/page/'.$nextpage.'/?page='.$nextpage; ?><?php if(isset($_REQUEST['q'])) { echo '&q='.$_REQUEST['q']; } ?><?php if(isset($_REQUEST['lat'])) { echo '&lat='.$_REQUEST['lat']; } ?><?php if(isset($_REQUEST['lon'])) { echo '&lon='.$_REQUEST['lon']; } ?><?php if(isset($_REQUEST['map'])) { echo '&map=1'; } ?>" class="next-page">next page</a><?php } ?>
			</div> 	
 	<?php
 	endif;	
...
?>
	...
	<?php
		global $sponsored; 
		if($total <= 3) { $sponsored = false; } else { $sponsored = true; }
		...
		if($sponsored == true) {
			...
			wp_reset_query(); $sponsored = false;
		}
	?> 
	...
	// endforeach;	
 	...
 	<div class="post_nav">
		<?php if($paged - 1 !== 0) { $previouspage = $paged-1; ?><a href="<?php echo get_bloginfo('url').'/golf-courses/'.$q.'/page/'.$previouspage; ?>" class="next-page">previous page</a><?php } ?>
		<?php if($paged < $search_query->max_num_pages) { $nextpage = $paged+1; ?><a href="<?php echo get_bloginfo('url').'/golf-courses/'.$q.'/page/'.$nextpage; ?>" class="next-page">next page</a><?php } ?>
	</div> 	
...
<?php else : ?>
		...
		<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>					
			<?php include('browse-courses.php'); ?>

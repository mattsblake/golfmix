...
function twentyten_comment( $comment, $args, $depth ) {
	...
	?>
		...
		<div class="reviews-center" id="review-<?php comment_ID(); ?>" itemprop="reviews" itemscope itemtype="http://schema.org/Review">
		<div class="recent-reviews-heading inner-recent-reviews-heading">
			
			<h4><?php printf( __( '<span itemprop="author">%s</span> <span>Wrote :</span>', 'twentyten' ), sprintf( '%s', get_comment_author() ) ); ?></h4>
			
			<div class="top-courses-col-3">
				<ul>
					<li><a  name="fb_share" share_url="<?php the_permalink($comment->post_ID);?>%23review-<?php comment_ID(); ?>" class="rss-link-3" rel="nofollow"></a></li>
					<li><a href="http://twitter.com/share?url=<?php the_permalink($comment->post_ID); ?>%23review-<?php comment_ID(); ?>&amp;text=Check+out+this+review+of+<?php the_title(); ?>&amp;via=golfmix" class="email-link-3" data-count="none" data-via="golfmix" target="_blank"></a></li>
					<li style="display:none;"><a href="<?php the_permalink($comment->post_ID); ?>%23review-<?php comment_ID(); ?>" class="refresh-link-3" rel="nofollow"></a></li>
				</ul>
			</div>
		</div>
		<div class="recent-post-box inner-recent-post-box">
		
		<div class="post-box-col-1 inner-span-1"> <span class="inner-span-1">
		<?php echo get_avatar( $comment->comment_author_email, 101 ); ?><!-- .comment-author .vcard -->
		...
		<div class="post-box-col-2 inner-span-2"> 
			<span class="clock-box inner-clock" itemprop="datePublished" content="<?php echo get_comment_date(); ?>">
		...
		<p itemprop="description"><?php //comment_text(); ?><?php echo nl2br($comment->comment_content) ?></p>
		</div>
			...
			<div class="new-reviwes-containder" id="addreview">
				...
				<?php else : ?>
					<form action="<?php echo site_url( '/wp-comments-post.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>">
						...
						<p class="form-submit">
							<input name="submit" type="submit" class="submit-area-inner" id="<?php echo esc_attr( $args['id_submit'] ); ?>" value="" />

...
if ($detect->isMobile() && !$detect->isIpad()) { ?>
	...
	<?php if(is_page('Search') || !stristr($this_uri, 'search') === FALSE) { ?>
...
<?php if(is_page('Search') || !stristr($this_uri, 'search') === FALSE) { ?><body onload="initialize()" onunload="GUnload()"><?php } else { ?><body><?php } ?>
...
<link rel="stylesheet" type="text/css" media="all" href="<?php bloginfo( 'stylesheet_url' ); ?>?v=2.2" />
...
<meta property="og:description"
      content="<?php if(is_page()) { the_title(); } elseif(is_single() && get_post_type() == 'post') { echo 'Reviews of '.get_the_title().', from regular golfers just like you.  Read & write your own reviews at www.golfmix.com.'; } elseif(is_single()) { setup_postdata($post); echo strip_tags(get_the_excerpt($post->ID)); } else { echo "golfmix is the ultimate resource to help you decide where to play golf in arizona.  read and write reviews of every course in the state.  whether you're a scratch or you shoot 115, there are golfers like you sharing their thoughts and ratings of tracks all across arizona.  join the movement!"; } ?>"/>
...
<?php 
if((is_single() && get_post_type() == 'post') || is_page('Search') || !stristr($this_uri, 'search') === FALSE) { ?><body onload="initialize()" onunload="GUnload()"><?php } else { ?><body><?php } ?>

<?php } ?>

<div id="wrapper">

 	<div id="location"><a href="<?php bloginfo('url'); ?>">Arizona</a></div>

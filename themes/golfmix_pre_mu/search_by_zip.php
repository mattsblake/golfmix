...
global $zip, $distance, $lat, $lon, $city, $q, $nearby;
...
if($zip) {
	...
	$wpdb->hide_errors();
	$nearby_courses = $wpdb->get_results( "SELECT POST_ID FROM courses WHERE LOCCITY LIKE '".$city_only."'", ARRAY_A); 
	...
	$wpdb->hide_errors();
	$nearby_courses = $wpdb->get_results( "SELECT POST_ID  FROM courses WHERE COMPANY LIKE '%".$q."%'", ARRAY_A); 

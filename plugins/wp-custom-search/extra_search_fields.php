...
class Field extends ParameterisedObject {
	...
	function getInput($name){
		...
		return "<li><span><input name='$htmlName' class='text-box' value='$value' onFocus='if(this.value=='(Address, City, State or Zip)'){this.value=''};' onBlur='if(this.value==''){this.value='(Address, City, State or Zip)'};'/></span></li>";
	}
...
class CustomSearchField extends SearchFieldBase {
	...
	function getInput(){
		return "<li>".$this->getLabel()."</li>".$this->input->getInput($this->name,$this->joiner);
	}
	function getCSSClass(){

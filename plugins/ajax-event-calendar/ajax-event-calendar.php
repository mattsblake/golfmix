...
if (!class_exists('ajax_event_calendar')) {
	class ajax_event_calendar {
		...
		function render_shortcode_calendar($atts) {
			...
			$out .= "</div>\n";
			$out .= "<div id='aec-calendar'></div>\n";
			$out .= $this->render_aec_version(true);
			$out .= "</div>\n";
			return $out;
		}
		...
		function render_eventlist_events($events, $whitelabel, $limit) {
			...
			foreach ($rows as $count => $row) {
				if ($count < $limit) {
					...
					if($row->start_time == '12:00 am') { $row->start_time = 'All Day';}

					// link to event
					$class = ($whitelabel) ? '' : ' ' . $row->className;
					
					if($row->className == 'cat1') { $type = 'Event'; }
					elseif($row->className == 'cat2') { $type = 'golfmixer'; }
					elseif($row->className == 'cat3') { $type = 'Tournament'; }
					else { $type = 'Event'; }

					
					$out .= '<div class="event"onClick="jQuery.aecDialog({\'id\':' . $row->id . ',\'start\':\'' . $row->start . '\',\'end\':\'' . $row->end . '\'});">
							<div class="event-date">'.$month.'<span>'.$day.'</span></div>
							<div class="event-details">
								<div class="date">'.$row->start_date.' - '.$row->start_time.'</div>
								<div class="type">'.$type.'</div>
								<div class="title">'.$this->render_i18n_data($row->title).'</div>
								<div class="description">'.$this->render_i18n_data($row->description).'</div>
							</div>
						</div>';
				}
			}
		...
		function add_select_field($field, $label, $options, $values=false) {
			$aec_options = get_option('aec_options');
			$out = "<label for='{$field}' class='semi'>{$label}</label>\n";
			$out .= "<select id='{$field}' name='aec_options[{$field}]'>";
			foreach ($options as $option => $value) {

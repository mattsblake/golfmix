...
add_action( 'plugins_loaded', 'remove_ev_internationalisation');
	...
	if ( is_wp_error($result) ) {
		if ( 'already_active' == $result->get_error_code() || 'blog_taken' == $result->get_error_code() ) {
		    ...
		    if( $signup->domain . $signup->path != '' ) {
		    	printf(__('<p class="lead-in">Your blog at <a href="%1$s">%2$s</a> is active. You may now login to your blog using your chosen username of "%3$s".  Please check your email inbox at %4$s for your password and login instructions.  If you do not receive an email, please check your junk or spam folder.  If you still do not receive an email within an hour, you can <a href="%5$s">reset your password</a>.</p>', 'removeev'), 'http://' . $signup->domain, $signup->domain, $signup->user_login, $signup->user_email, 'http://' . $current_site->domain . $current_site->path . 'login?action=lostpassword');
		    }
		} else {
			?>
		...
		<?php if( !empty($url) ) : ?>
			<p class="view"><?php printf(__('You\'re all set up and ready to go. <a href="%1$s">View your site</a> or <a href="%2$s">Login</a>', 'removeev'), $url, $url . 'login' ); ?></p>
		<?php else: ?>
			<p class="view"><?php printf( __( 'You\'re all set up and ready to go. <a href="%1$s">Login</a> or go back to the <a href="%2$s">homepage</a>.', 'removeev' ), 'http://' . $current_site->domain . $current_site->path . 'login', 'http://' . $current_site->domain . $current_site->path ); ?></p>
		<?php endif;
	}
...
function activate_on_user_signup($user, $user_email, $key, $meta) {
	...
	if ( is_wp_error($result) ) {
		...
		$html .= '<p class="view">' . sprintf( __( 'You can now update your details by <a href="%1$s">Logging in</a> to your account or go back to the <a href="%2$s">homepage</a>.', 'removeev' ), 'http://' . $current_blog->domain . $current_blog->path . 'login', 'http://' . $current_blog->domain . $current_blog->path ) . '</p>';
